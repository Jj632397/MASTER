...
!-------------------------------------------------------------------------------
  ...
  integer, private, parameter :: iact_llf = 0
...
!-------------------------------------------------------------------------------
    subroutine calc_fvs(cd1,cd2,cd3)
      ...
      real(8) :: eig1max,eig2max,eig3max,eig4max,eig5max,eigmax
      ...
      do jj=1,jjmax+1
        ...
        do ii=1,iimax
          ...
          !maximum eigenvalues
          eig1max = dmax1(dabs(eig1l),dabs(eig1m),dabs(eig1r))
          eig2max = dmax1(dabs(eig2l),dabs(eig2m),dabs(eig2r))
          eig3max = dmax1(dabs(eig3l),dabs(eig3m),dabs(eig3r))
          eig4max = dmax1(dabs(eig4l),dabs(eig4m),dabs(eig4r))
          eig5max = dmax1(dabs(eig5l),dabs(eig5m),dabs(eig5r))

          eigmax = dmax1(eig1max,eig2max,eig3max,eig4max,eig5max)

          eig1max = eigmax
          eig2max = (1.0d0-cd1)*eig1max+cd1*eigmax
          eig3max = (1.0d0-cd2)*eig1max+cd2*eigmax
          eig4max = (1.0d0-cd3)*eig1max+cd3*eigmax
          eig5max = eigmax
          ...
          if (iact_llf==1) then
            ...
            dq4 = eigmax*dq4
            dq5 = eigmax*dq5
            if (neqns>=6) dq6 = eigmax*dq6
            if (neqns>=7) dq7 = eigmax*dq7
            if (neqns>=8) dq8 = eigmax*dq8
            if (neqns>=9) dq9 = eigmax*dq9
            if (neqns>=10)dq10= eigmax*dq10
            if (neqns>=11)dq11= eigmax*dq11
            if (neqns>=12)dq12= eigmax*dq12
          else
            ...
            ds4 = eig4max*ds4
            ds5 = eig5max*ds5
            if (neqns>=6) ds6 = eig1max*ds6
            if (neqns>=7) ds7 = eig1max*ds7
            if (neqns>=8) ds8 = eig1max*ds8
            if (neqns>=9) ds9 = eig1max*ds9
            if (neqns>=10)ds10= eig1max*ds10
            if (neqns>=11)ds11= eig1max*ds11
            if (neqns>=12)ds12= eig1max*ds12
