...
!-------------------------------------------------------------------------------
  ...
  real(8), private, parameter :: coef_thermo_diss = 0.0d0
...
!-------------------------------------------------------------------------------
    subroutine calc_fvs(cd1,cd2,cd3)
      ...
      real(8) :: ys1m,ys2m,ys3m,ys4m,ys5m,ys6m,ys7m
      ...
      do jj=1,jjmax+1
        ...
        do ii=1,iimax
          ...
          wm  = 0.5d0*( wl+ wr)
          htm = 0.5d0*(htl+htr)
          if (neqns>=6) ys1m = 0.5d0*(ys1l+ys1r)
          if (neqns>=7) ys2m = 0.5d0*(ys2l+ys2r)
          if (neqns>=8) ys3m = 0.5d0*(ys3l+ys3r)
          if (neqns>=9) ys4m = 0.5d0*(ys4l+ys4r)
          if (neqns>=10)ys5m = 0.5d0*(ys5l+ys5r)
          if (neqns>=11)ys6m = 0.5d0*(ys6l+ys6r)
          if (neqns>=12)ys7m = 0.5d0*(ys7l+ys7r)
          ...
          ds1 = dq1+alph*(c2m/vp2m-1.0d0)/(rhm*hstm)*dq5
          ds2 = dq2
          ds3 = dq3
          ds4 = dq4
          ds5 = c2m/vp2m*dq5
          ...
          dq1 =    rhtm*ds1+rhpm*ds5
          dq2 = um*rhtm*ds1+rhm*ds2+um*rhpm*ds5
          dq3 = vm*rhtm*ds1+rhm*ds3+vm*rhpm*ds5
          dq4 = wm*rhtm*ds1+rhm*ds4+wm*rhpm*ds5
          dq5 = (rhtm*htm+rhm*hstm)*ds1+um*rhm*ds2+vm*rhm*ds3+wm*rhm*ds4+(htm*rhpm-alph)*ds5
          if (neqns>=6) dq6 = ys1m*rhtm*ds1+ys1m*rhpm*ds5+rhm*ds6
          if (neqns>=7) dq7 = ys2m*rhtm*ds1+ys2m*rhpm*ds5+rhm*ds7
          if (neqns>=8) dq8 = ys3m*rhtm*ds1+ys3m*rhpm*ds5+rhm*ds8
          if (neqns>=9) dq9 = ys4m*rhtm*ds1+ys4m*rhpm*ds5+rhm*ds9
          if (neqns>=10)dq10= ys5m*rhtm*ds1+ys5m*rhpm*ds5+rhm*ds10
          if (neqns>=11)dq11= ys6m*rhtm*ds1+ys6m*rhpm*ds5+rhm*ds11
          if (neqns>=12)dq12= ys7m*rhtm*ds1+ys7m*rhpm*ds5+rhm*ds12

          FLUX_ALN(ii,jj,1) = 0.5d0*(f1l+f1r-dq1)
          FLUX_ALN(ii,jj,2) = 0.5d0*(f2l+f2r-dq2)
          FLUX_ALN(ii,jj,3) = 0.5d0*(f3l+f3r-dq3)
          FLUX_ALN(ii,jj,4) = 0.5d0*(f4l+f4r-dq4)
          FLUX_ALN(ii,jj,5) = 0.5d0*(f5l+f5r-dq5)
          if (neqns>=6) FLUX_ALN(ii,jj,6) = 0.5d0*(f6l+f6r-dq6)
          if (neqns>=7) FLUX_ALN(ii,jj,7) = 0.5d0*(f7l+f7r-dq7)
          if (neqns>=8) FLUX_ALN(ii,jj,8) = 0.5d0*(f8l+f8r-dq8)
          if (neqns>=9) FLUX_ALN(ii,jj,9) = 0.5d0*(f9l+f9r-dq9)
          if (neqns>=10)FLUX_ALN(ii,jj,10)= 0.5d0*(f10l+f10r-dq10)
          if (neqns>=11)FLUX_ALN(ii,jj,11)= 0.5d0*(f11l+f11r-dq11)
          if (neqns>=12)FLUX_ALN(ii,jj,12)= 0.5d0*(f12l+f12r-dq12)
        enddo
      enddo
